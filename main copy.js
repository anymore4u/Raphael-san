...
// Função para iniciar o bot após a conexão com o MongoDB
const startBot = () => {
    ...
    client.on('messageCreate', async message => {
        if (message.content.startsWith(';porvalope')) {
            ...
            if (args[1] === 'roleta') {
                ...
                const cooldownTime = 3600; // Cooldown time in seconds (e.g., 1 hour)
                ...
                if (cooldown && cooldown.expiresAt > now) {
                    ...
                    return message.reply(`Você está em cooldown. Tente novamente em ${timeLeft} segundos.`);
                }

                const userIds = [
                    '560444018622857247', '225331823742418944', '339530417474371585',
                    '246684210960531458', '539596207849340948', '446434441804513338',
                    '128346888863875072', '518669408554713108', '380826553329909763',
                    '159075102296768512', '393581900155060224', '302252030922326017',
                    '491459349957312512', '346295489911717889'
                ];
                    ...
                    try {
                        ...
                        if (member) {
                            ...
                            const newCooldown = {
                                userId,
                                command: commandName,
                                expiresAt: new Date(now.getTime() + cooldownTime * 1000)
                            };
                            await db.collection('cooldowns').insertOne(newCooldown);
    ...
    client.on('messageCreate', async message => {
        ...
        if (/\b(GAY|YAG|GAE|GUEI|HOMOSEXUAL)\b/i.test(message.content.toUpperCase())) {
            // Lista de mensagens possíveis
            const messages = [
                'tão de falando de vc <@446434441804513338>',
                'tão de falando de vc <@393581900155060224>'
            ];
        ...
        } else if (/\b(BAITOLA|CHORÃO|CHORAO|SÓ FALA BOSTA|MACONHEIRO|NOIA)\b/i.test(message.content.toUpperCase())) {
            const msg = 'tão de falando de vc <@393581900155060224>';
            await message.reply(msg);
        } else if (/\b(WHALE|BEBADO|TARTARUGA SLAYER)\b/i.test(message.content.toUpperCase())) {
            const msg = 'tão de falando de vc <@491459349957312512>';
            await message.reply(msg);
            ...
            const msg = 'tão de falando de vc <@560444018622857247>';
            await message.reply(msg);
        } else if (/\b(NECROFILO|LOLICON|INCESTO)\b/i.test(message.content.toUpperCase())) {
            const msg = 'tão de falando de vc <@380826553329909763>';
            await message.reply(msg);
        } else if (/\b(BEBADA)\b/i.test(message.content.toUpperCase())) {
            const msg = 'tão de falando de vc <@159075102296768512>';
            await message.reply(msg);
            ...
            const randomUserId = userIds[Math.floor(Math.random() * userIds.length)];
            const msg = `tão de falando de vc <@${randomUserId}>`;
            await message.reply(msg);
        }
    });
    ...
    client.on('messageCreate', async message => {
        ...
        if (message.content.startsWith(';loopporvalope')) {
...
// Conectar ao MongoDB e iniciar o bot
connectMongo().then(() => {
    startBot();
});
